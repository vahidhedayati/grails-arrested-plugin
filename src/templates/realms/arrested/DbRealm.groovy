...
class DbRealm {
    ...
    def authenticate(authToken) {
        ...
        def account = new SimpleAccount(username, user.passwordHash,
			new org.apache.shiro.util.SimpleByteSource(user.passwordSalt), "DbRealm")
    ...
    def hasRole(principal, roleName) {
        def roles = ArrestedUser.withCriteria {
            roles {
                eq("name", roleName)
            }
    ...
    def hasAllRoles(principal, roles) {
        def r = ArrestedUser.withCriteria {
            roles {
                'in'("name", roles)
            }
            eq("username", principal)
    ...
    def isPermitted(principal, requiredPermission) {
        ...
        def user = ArrestedUser.findByUsername(principal)
        ...
        def results = ArrestedUser.executeQuery("select distinct p from ArrestedUser as user join user.roles as role join role.permissions as p where user.username = '$principal'")
